...
export function Hero() {
  ...
  const [step, setStep] = useState(0); // 0: Email, 1: Name
    ...
    <section className="relative mt-6 pt-8 md:pt-12 lg:pt-16 pb-10 md:pb-16 border-b border-border/50 bg-background overflow-hidden">
      ...
      >
        ...
        <div className="flex flex-col items-start text-left w-full">
          ...
          <motion.div
            variants={itemVariants}
            className="pl-5 border-l-2 border-primary mb-12"
          >
            <p className="text-lg md:text-xl text-muted-foreground max-w-xl leading-relaxed font-light">
              Z is your AI study companion — built for university students who
              want to master their curriculum, ace their exams, and stay ahead
              of the class without the guesswork.
            </p>
          </motion.div>

          <motion.div
            variants={itemVariants}
            className="w-full max-w-lg scroll-mt-32"
            id="waitlist-form"
          >
            <AnimatePresence mode="wait">
              {!submitted ? (
                      ...
                      {step === 0 ? (
                        ...
                        >
                          <Input
                            ...
                            className="flex-1 bg-transparent border-border/50 rounded-lg uppercase font-mono text-xs placeholder:text-muted-foreground/40 h-14 px-4 focus-visible:ring-1 focus-visible:ring-primary focus-visible:border-primary tracking-wider"
                          />
                          <motion.button
                            type="button"
                            onClick={() => handleNext()}
                            ...
                            className="w-14 h-14 rounded-lg bg-primary text-primary-foreground hover:bg-white hover:text-primary hover:ring-1 hover:ring-inset hover:ring-primary flex items-center justify-center transition-all duration-300 disabled:opacity-30 group cursor-pointer disabled:cursor-not-allowed"
                          >
                        ...
                        >
                          ...
                          <motion.button
                            ...
                            whileHover={
                              formData.name.length >= 2 ? { scale: 1.05 } : {}
                            }
                            whileTap={
                        ...
                        >
                          <Input
                            type="text"
                            placeholder={
                              isPending ? "SYNCING..." : "CONFIRM FULL NAME"
                            }
                            required
                            autoFocus
                            disabled={isPending}
                            ...
                            className={`flex-1 bg-transparent border-border/50 rounded-lg uppercase font-mono text-xs placeholder:text-muted-foreground/40 h-14 px-4 focus-visible:ring-1 focus-visible:ring-primary focus-visible:border-primary tracking-wider ${isPending ? "animate-pulse border-primary/50" : ""}`}
                          />
                          <motion.button
                            type="button"
                            onClick={() => handleNext()}
                            disabled={formData.name.length < 2 || isPending}
                            whileHover={
                              formData.name.length >= 2 && !isPending
                                ? { scale: 1.05 }
                                : {}
                            }
                            whileTap={
                ...
                <motion.div
                  ...
                  className="bg-primary/5 border border-primary/20 text-primary px-6 py-6 rounded-lg font-mono text-sm tracking-wider uppercase font-bold text-center"
                >
                  ...
                  <div className="text-[10px] p-3 bg-primary/10 border border-primary/20 animate-pulse text-primary/80 font-normal">
                    NOTE: IF YOU DON&apos;T SEE A CONFIRMATION EMAIL, CHECK YOUR
                    SPAM FOLDER OR WAIT A FEW MINUTES.
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
          ...
          <motion.div
            variants={itemVariants}
            className="flex flex-wrap items-center gap-6 sm:gap-10 mt-10 font-mono"
          >
            <div>
              <div className="text-foreground font-bold text-lg sm:text-xl uppercase tracking-wider mb-0.5">
                100%
              </div>
              <div className="text-muted-foreground text-[10px] tracking-[0.15em] uppercase">
                Curriculum Sync
              </div>
            </div>
