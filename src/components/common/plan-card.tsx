...
export const PLAN_TIER_META: Record<
  string,
  { label: string; tagline: string; color: string; borderHover: string }
> = {
  cooked: {
    label: "Cooked",
...
export function getPlanFeaturesList(pkg: BillingPackage) {
  ...
  if (pkg.tier === "locked_in")
    f.push("Priority processing", "Early feature access");
