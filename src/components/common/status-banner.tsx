 ...
 *
 * Restyled to match the Qz marketing surface: rounded-[36px] hero card,
 * soft hand-written side caption, landing-style blob accents.
 */
...
export function StatusBanner({ state, label, generatedAt }: StatusBannerProps) {
  ...
  return (
    <div
      className={`relative mt-6 overflow-hidden rounded-[36px] border ${copy.accent} ${copy.bg} px-6 py-7 sm:px-9 sm:py-9`}
      style={{ borderRadius: "36px" }}
    >
      {/* Soft blob accents — landing convention */}
      <div className="pointer-events-none absolute -right-16 -top-24 h-64 w-64 rounded-full bg-blue-100/40 blur-3xl" />
      <div className="pointer-events-none absolute -bottom-20 -left-10 h-48 w-48 rounded-full bg-[#0C60FC]/5 blur-3xl" />
      ...
      <div className="relative grid items-center gap-6 sm:grid-cols-[1fr_auto]">
        ...
        <div className="relative shrink-0">
          <span className="hand absolute -left-24 -top-3 hidden w-24 -rotate-6 text-base leading-5 text-[#0C60FC] sm:block">
            {copy.sideCaption}
          </span>
