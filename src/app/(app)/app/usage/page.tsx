...
const FEATURE_ROWS: Array<{
  ...
  {
    key: "tutorSessions",
    limitKey: "tutorSessionsPerDay",
    icon: Brain,
    label: "Z Tutor Sessions",
  },
  {
    key: "quizGenerations",
    limitKey: "quizGenerationsPerDay",
    icon: BookOpen,
    label: "Quiz Generations",
  },
  {
    key: "flashcardSets",
    limitKey: "flashcardSetsPerDay",
    icon: Zap,
    label: "Flashcard Sets",
  },
  {
    key: "mindMaps",
    limitKey: "mindMapsPerDay",
    icon: FileText,
    label: "Mind Maps",
  },
  {
    key: "materialUploads",
    limitKey: "materialUploadsPerDay",
    icon: Upload,
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <Icon
          className={cn(
            "size-4",
            accent ? "text-primary/70" : "text-muted-foreground/30",
          )}
        />
      </div>
      <p
    ...
    >
      ...
      <div className="flex flex-col gap-3">
        {FEATURE_ROWS.map(({ key, limitKey, icon: Icon, label }) => {
          ...
          const limit = limits
            ? (limits[limitKey as keyof typeof limits] as number | null)
            : 2;
...
function PhaseFunnel({
  ...
  const phases = PHASE_ORDER.map((p) => ({
    ...p,
    count: sessionsByPhase[p.key] ?? 0,
  })).filter((p) => p.count > 0);

  const max = Math.max(...phases.map((p) => p.count), 1);

  return (
    <div className="border border-border/50 bg-card/40 p-5 rounded-lg">
...
export default function UsagePage() {
  ...
  const isPaidTier =
    isSuperAdmin ||
    billing?.planTier === "cruising" ||
    billing?.planTier === "locked_in";
                ...
                {billing?.planTier ? (
                  <>
                    ...
                    <p className="text-xs font-mono text-muted-foreground">
                      {DURATION_LABELS[billing.planDuration!] ??
                        billing.planDuration}
                  ...
                  <>
                    <p className="text-xl font-black tracking-tighter text-muted-foreground">
                      Free
                    </p>
                    <button
            ...
            {isPaidTier && analytics ? (
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="border border-border/50 bg-card/40 px-4 py-4 flex flex-col gap-2 rounded-lg">
                    <div className="flex items-center justify-between">
                  ...
                  <div className="border border-border/50 bg-card/40 px-4 py-4 flex flex-col gap-2 rounded-lg">
                    ...
                    <p className="text-2xl font-black tracking-tighter">
                      {analytics.avgSessionDurationMinutes}m
                    </p>
                    <p className="text-[9px] font-mono text-muted-foreground/40">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <ArtifactBreakdown
                    artifactsByType={analytics.artifactsByType}
                  />
                  <SatisfactionBar
              ...
              !isPaidTier && (
                ...
                >
                  ...
                  <p className="text-sm text-muted-foreground mb-3">
                    Phase funnels, artifact breakdowns, and session stats are
                    available on Cruising and Locked In plans.
                  </p>
                  <button
                    onClick={() => router.push("/app/billing")}
