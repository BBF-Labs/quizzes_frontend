...
export default function StudyRoomDetailPage() {
  ...
  const myParticipant = useMemo(() => {
    ...
    return (room?.participants || []).find(
      (p) =>
        (myId && p.userId === myId) || (myGuestId && p.guestId === myGuestId),
    );
  ...
  useEffect(() => {
    if (
      !isHost ||
      !room?.sharedMedia?.status ||
      room.sharedMedia.status !== "playing"
    )
      return;
  ...
  const optionCounts = useMemo(() => {
    ...
    return game.options.map(
      (_, idx) => game.responses!.filter((r) => r.optionIndex === idx).length,
    );
  ...
  const handleSubmitAnswer = () => {
    ...
    submitGameAnswer.mutate(
      { code, answer, guestId: gId },
      {
        ...
        onError: (e: any) =>
          toast.error(e?.message || "Could not submit answer"),
      },
    );
  };
            ...
            <CardContent className="p-5">
              <div className="flex items-center justify-between gap-2 border-b border-border/50 pb-4">
                <div className="min-w-0">
                  <h2 className="truncate font-mono text-sm font-bold italic tracking-tight text-foreground">
                    {room.title}
                  </h2>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      generateAiGame.mutate(
                        { code, type: "word_guess" },
                        {
                          onSuccess: () =>
                            toast.success("Z is drafting a word challenge…"),
                          onError: (e: any) =>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      generateAiGame.mutate(
                        ...
                        {
                          onSuccess: () =>
                            toast.success("Z is preparing a Q&A battle…"),
                          onError: (e: any) =>
                            toast.error(e?.message || "Generation failed"),
                        },
                      )
                  ...
                  {leaderboard.map((p, i) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={cn(
                            "w-4 shrink-0 text-xs font-mono font-black italic",
                        ...
                        <img
                          src={buildAvatarUri(p.displayName, p.avatarConfig)}
                          className="size-8 rounded-full border border-border/50 bg-muted shrink-0"
                          alt=""
                        />
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1">
                            <p className="truncate text-[11px] font-mono font-bold uppercase">
                              {p.displayName}
                            </p>
                            {p.role === "host" && (
                            ...
                            {p.role === "moderator" && (
                              <Shield className="size-2.5 shrink-0 text-blue-400" />
                            )}
                          </div>
                          ...
                          <DropdownMenu>
                            ...
                            >
                              ...
                              {p.userId && p.role !== "moderator" && (
                                ...
                                >
                                  <Shield className="mr-2 size-3 text-blue-400" />{" "}
                                  Promote to Mod
                                </DropdownMenuItem>
                              )}
                              <DropdownMenuItem
                                onClick={() =>
                                  moderateMember.mutate(
                                    ...
                                    {
                                      onSuccess: () =>
                                        toast.success(`${p.displayName} muted`),
                                      onError: (e: any) =>
                                        toast.error(e?.message || "Failed"),
                                    },
                                  )
                              ...
                              >
                                <MicOff className="mr-2 size-3 text-amber-500" />{" "}
                                Mute
                              </DropdownMenuItem>
                              <DropdownMenuItem
              ...
              <div className="flex flex-col items-center gap-5">
                      ...
                      {participants.length === 1
                        ...
                        : participants.length <= 3
                          ? `${participants
                              ...
                              .join(
                                ", ",
                              )} and ${participants[participants.length - 1].displayName} are studying`
                ...
                {isHost && !room.timer?.isRunning && (
                  <div className="flex items-center gap-1.5">
                    <p className="font-mono text-[9px] text-muted-foreground uppercase tracking-widest mr-1">
                      Duration
                    </p>
                ...
                <div className="flex items-center gap-3">
                  {isHost && (
                    <>
                      ...
                      <button
                        onClick={() =>
                          updateTimer.mutate({ code, action: "reset" })
                        }
                  ...
                  <CardContent className="p-6">
                      ...
                      <div className="flex flex-col items-center gap-4 py-8 text-center">
                        ...
                        <div>
                          <p className="font-mono text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                            Z is generating your challenge…
                          </p>
                        </div>
                      </div>
                    )}
                      ...
                      <div className="flex flex-col gap-4">
                        ...
                        {!isHost && (
                          <p className="text-[10px] font-mono text-muted-foreground uppercase">
                            Waiting for host to launch the challenge…
                          </p>
                        )}
                        {isHost && (
                    ...
                    {game.status === "running" && isQa && (
                      <div className="flex flex-col gap-5">
                        ...
                        <p className="font-mono text-sm font-bold leading-snug text-foreground">
                          {game.prompt}
                        </p>
                ...
                <Card className="rounded-lg border-primary/40 overflow-hidden shadow-lg">
                  ...
                  <CardContent className="p-6 text-center space-y-5">
                    <div>
                      ...
                      <h3 className="mt-2 font-mono text-lg font-black italic uppercase tracking-tight">
                        How did that go?
                      </h3>
                    </div>
                    <div className="grid gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                {room.sharedMedia?.updatedByName && (
                  <span className="text-[9px] font-mono text-muted-foreground">
                    — cast by {room.sharedMedia.updatedByName}
                  </span>
                )}
              </div>
              {isHost && (
              ...
              {!videoId && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-muted-foreground/30">
                  ...
                  <p className="font-mono text-[10px] font-bold uppercase tracking-widest">
                    {isHost
                      ? "Paste a YouTube URL above to cast to the room"
                      : "Waiting for host to cast media…"}
                  </p>
                </div>
              )}
            </div>
                      ...
                      <Input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter" && taskTitle) {
                            createTask.mutate(
                              { code, title: taskTitle, points: 50 },
                              {
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-mono text-[10px] text-muted-foreground">
                      {leaderboard.find((p) => p.displayName === myDisplayName)
                        ?.points ?? 0}{" "}
                      XP
                      {" · "}Rank #
                ...
                <Dialog>
                  ...
                  <DialogContent className="w-full max-w-[95vw] rounded-lg border-border/50 bg-background shadow-2xl p-0 sm:max-w-xl">
                    ...
                    <AvatarBuilder
                      initialConfig={
                        myParticipant?.avatarConfig as
                          | Record<string, string>
                          | undefined
                      }
                      onUpdate={(c) =>
                        updateAvatar.mutate({ code, avatarConfig: c })
                      }
                    />
                  </DialogContent>
                </Dialog>
        ...
        {room.readyState?.isOpen && (
          <Button
            ...
            onClick={async () => {
              ...
              await toggleGameReady.mutateAsync({
                code,
                ready: !amIReady,
                guestId,
              });
