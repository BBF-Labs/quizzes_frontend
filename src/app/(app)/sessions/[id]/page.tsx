"use client";
// Force HMR refresh - userMessageObj is definitely removed.

import { useCallback, useEffect, useRef, useState } from "react";
import { use } from "react";
import { useRouter } from "next/navigation";
import { motion, AnimatePresence } from "framer-motion";
import { Loader2, Paperclip, X, Settings2, Plus, Sparkles, ArrowUp } from "lucide-react";
...
export default function ChatPage({ params }: SessionPageProps) {
  ...
  // Transform session messages from backend to ZSessionMessage[]
  const initialMessages = (session?.zMessages || [])
    .filter((msg) => msg.type === "text" || msg.type === "thinking")
    .map((msg) => ({
      id: msg.messageId || msg.id,
      messageId: msg.messageId || msg.id,
      role: msg.role === "user" ? ("user" as const) : ("z" as const),
      type: (msg.type === "thinking" ? "thinking" : "text") as "text" | "thinking",
      content: msg.content,
      timestamp: msg.timestamp,
    }));

  const stream = useSessionStream(sessionId, initialMessages);
  ...
  const handleSend = useCallback(async () => {
    ...
    // Add user message to UI (optimistic update via hook)
    stream.pushMessage({
      id: userMessageId,
      role: "user",
      type: "text",
  ...
  }, [input, stepMutation, sessionId, stream]);
    ...
    <div className="flex h-screen flex-col bg-background overflow-hidden">
      ...
      {!sessionId || sessionId === "undefined" ? (
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          {/* Messages Section */}
          <div className="flex flex-1 flex-col relative min-h-0 min-w-0">
              ...
              <div className="w-full flex flex-col gap-10">
                {messages.length === 0 ? (
                  ...
                  >
                    ...
                    <div>
                      <h2 className="text-xl font-mono uppercase tracking-[0.3em] font-bold mb-3">Study Partner</h2>
                      <p className="text-muted-foreground max-w-sm">
                        I&apos;m here to help you study, explain concepts, and prepare for your exams.
                      </p>
                    </div>
                  </motion.div>
                ) : (
                ...
                {stream.isThinking && !!stream.thinkingBuffer && (
            ...
            {/* Sidebar / Materials */}
            {session && (
              <aside className="hidden lg:block w-80 border-l border-border/50 bg-card/20 p-4 shrink-0 overflow-y-auto scrollbar-none">
                <MaterialManager
                  sessionId={sessionId}
